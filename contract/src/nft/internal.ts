...
export function refundApprovedAccountIdsIter(
  ...
  const storageReleased = approvedAccountIds
...
export function refundDeposit(storageUsed: bigint) {
  ...
  const requiredCost = storageUsed * near.storageByteCost().valueOf();
  ...
  const refund = attachedDeposit - requiredCost;
...
export function internalAddTokenToOwner(
  ...
  let tokenSet = restoreOwners(
    contract.tokensPerOwner.get(accountId) as UnorderedSet | null,
  );
...
export function internalTransfer(
  ...
  if (senderId != token.owner_id) {
    ...
    if (approvalId) {
      ...
      const actualApprovalId = token.approved_account_ids[senderId];
