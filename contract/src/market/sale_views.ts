...
export function internalSupplyByOwnerId({
  ...
  const byOwnerId = restoreOwners(
    contract.byOwnerId.get(accountId) as UnorderedSet | null,
  );
...
export function internalSalesByOwnerId({
  ...
  for (let i = start; i < max; i++) {
    ...
    const sale = contract.sales.get(keys[i]) as Sale;
    if (sale != null) {
      sales.push(sale);
    }
  }
...
export function internalSupplyByNftContractId({
  ...
  const byNftContractId = restoreOwners(
    contract.byNftContractId.get(nftContractId) as UnorderedSet | null,
  );
  //if there as some set, we return the length but if there wasn't a set, we return 0
...
export function internalSalesByNftContractId({
  ...
  const tokenSet = restoreOwners(
    contract.byNftContractId.get(accountId) as UnorderedSet | null,
  );
