...
export function internalSupplyByOwnerId({
    ...
    const byOwnerId = restoreOwners(contract.byOwnerId.get(accountId) as UnorderedSet | null);
...
export function internalSalesByOwnerId({
    ...
    let tokenSet = restoreOwners(contract.byOwnerId.get(accountId) as UnorderedSet | null);
...
export function internalSupplyByNftContractId({
    ...
    let byNftContractId = restoreOwners(contract.byNftContractId.get(nftContractId) as UnorderedSet | null);
    //if there as some set, we return the length but if there wasn't a set, we return 0
...
export function internalSalesByNftContractId({
    ...
    let tokenSet = restoreOwners(contract.byNftContractId.get(accountId) as UnorderedSet | null);
