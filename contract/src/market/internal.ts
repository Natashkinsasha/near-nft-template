...
export function internallyRemoveSale(contract: Contract, nftContractId: string, tokenId: string): Sale {
    ...
    let byOwnerId = restoreOwners(contract.byOwnerId.get(sale.owner_id) as UnorderedSet);
    if (byOwnerId == null) {
    ...
    let byNftContractId = restoreOwners(contract.byNftContractId.get(nftContractId) as UnorderedSet);
