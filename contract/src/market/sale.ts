...
export type Sale = {
  //owner of the sale
  owner_id: string;
  //market contract's approval I D to transfer the token on behalf of the owner
  approval_id: number;
...
export function internalRemoveSale({
  ...
  const sale = internallyRemoveSale(contract, nftContractId, tokenId);
...
export function internalUpdatePrice({
  ...
  const sale = contract.sales.get(contractAndTokenId) as Sale;
  if (sale == null) {
...
export function internalOffer({
  ...
  const sale = contract.sales.get(contractAndTokenId) as Sale | null;
  if (!sale) {
    throw new Error('no sale');
  }
  ...
  const price = BigInt(sale.sale_conditions);
...
export function processPurchase({
  ...
  return NearPromise.new(nftContractId)
    .functionCall(
      'nft_transfer_payout',
      bytes(
        JSON.stringify({
...
export function internalResolvePurchase({
  ...
  if (success && result) {
    ...
    const payoutOption = JSON.parse(result);
    if (
      !(
      ...
      Object.entries(payoutOption.payout).forEach(([, value]) => {
      ...
      if (remainder == BigInt(0) || remainder == BigInt(1)) {
        ...
        near.log(`error parsing payout object: Payout is not correct`);
      }
    } else {
      //we'll check if length of the payout object is > 10 or it's empty. In either case, we return None
      near.log(
        `error parsing payout object: Cannot have more than 10 royalties`,
      );
