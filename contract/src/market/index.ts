import {NearBindgen, near, call, view, LookupMap, UnorderedMap, assert, UnorderedSet} from 'near-sdk-js'
...
export class Contract {
    ...
    */
    sales: UnorderedMap = new UnorderedMap("sales");
    
    //keep track of all the Sale IDs for every account ID
    byOwnerId: LookupMap = new LookupMap("byOwnerId");

    //keep track of all the token IDs for sale for a given contract
    byNftContractId: LookupMap = new LookupMap("byNftContractId");

    //keep track of the storage that accounts have payed
    storageDeposits: LookupMap = new LookupMap("storageDeposits");
    ...
    */
    ...
    storage_deposit({ account_id }: { account_id?: string }) {
        ...
        const storageAccountId = account_id ?? near.predecessorAccountId();
        ...
        const balance: string = this.storageDeposits.get(storageAccountId) as string || "0";
        //add the deposit to their balance
        const newBalance = BigInt(balance) + deposit;
    ...
    storage_withdraw() {
        ...
        const ownerId = near.predecessorAccountId();
        //get the amount that the user has by removing them from the map. If they're not in the map, default to 0
        const amount: string = this.storageDeposits.remove(ownerId) as string || "0";
        
        //how many sales is that user taking up currently. This returns a set
        const sales = restoreOwners(this.byOwnerId.get(ownerId) as UnorderedSet | null);
        ...
        const diff = BigInt(len) * STORAGE_PER_SALE;
        //the excess to withdraw is the total storage paid - storage being used up.
        const amountLeft = BigInt(amount) - diff;
    ...
    */
    ...
    nft_on_approve({token_id, owner_id, approval_id, msg}:{token_id: string, owner_id: string, approval_id: number, msg: string}): void {
